...
On scalar context, the first element is only retruned.
=head2 @elem = $html->getElementsByAttributeValueContaining( $attrname, $valueFragment );

This method returns an array of elements which $attrname attribute's value contains $valueFragment.
On scalar context, the first element is only retruned.
...
sub getElementsByAttributeValueContaining {
    my $self = shift;
    my $key  = lc(shift);
    my $val  = shift;
